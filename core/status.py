...
def create_status_app(connection: BaseConnection):
    ...
    @app.command("show")
    def show_config(interface_name: str, mac: Annotated[bool, typer.Option("--mac")] = False):
        ...
        if mac:
            ...
            raise typer.Abort()
        output = connection.send_command(f"show configuration interfaces {interface_name}")
    ...
    def show_all_lacp(lacp_name: str):
        ...
        print(connection.send_command("show configuration interfaces | display set relative | match aggregated-ether-options | no-more"))
    ...
    def show_status(interface_name: str):
        ...
        if not is_valid_interface(interface_name):
            print("Error! Name of interface should be ge-(0-9)/(0-9)/(0-9)")
            raise typer.Abort()
        output = connection.send_command(f"show interfaces {interface_name} terse | no-more")
        print(output)
