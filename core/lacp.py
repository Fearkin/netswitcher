...
def create_lacp_app(connection: BaseConnection):
    ...
    def create(
        ...
        if port < 0:
            ...
            raise typer.Abort()
        if not (is_valid_interface(first_interface) and is_valid_interface(second_interface)):
            print("Error! Name of interface should be ge-(0-9)/(0-9)/(0-9)")
            raise typer.Abort()
        
        connection.send_command("configure")
        connection.send_command(f"set interfaces ae{port} aggregated-ether-options lacp active")
        connection.send_command(f"set interfaces ae{port} aggregated-ether-options lacp periodic fast")
        connection.send_command(f"set interfaces ae{port} description \"{description}\"")
        connection.send_command(f"set interfaces ae{port} unit 0 family ethernet-switching")
            
        for i in (first_interface, second_interface):
            connection.send_command(f"delete interfaces {i} unit 0")
            connection.send_command(f"set interfaces {i} ether-options 802.3ad ae{port}")
    ...
    def remove(interface_name: str):
        ...
        connection.send_command(f'set interfaces {interface_name} unit 0 family ethernet-switching')
        connection.send_command(f'set interfaces {interface_name} unit 0 family ethernet-switching storm-control default')
